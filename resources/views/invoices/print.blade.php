...
<head>
  ...
  <title>SEN SOLUTION ELECTRONIQUE — Facture {{ $invoice->invoice_number }}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header-stripe {
      height: 6px;
      background: linear-gradient(90deg, #FF6F00, #FFB300, #FFF176, #FFB300, #FF6F00);
    }
    ...
    .items-table tbody td.ref      { font-size: 11px; color: #7986cb; font-weight: 500; }
    ...
    .totals-line {
      ...
      justify-content: space-between;
      padding: 7px 0;
      border-bottom: 1px dashed #e0e4f4;
      font-size: 13px;
      color: #4a5580;
    }
...
<div class="page">
  ...
  <div class="meta-band">
    ...
    <div class="meta-block right">
      ...
      <div class="date-badge" style="margin-left:auto;">
        <span class="day">{{ $invoice->issued_at->format('d') }}</span>
        <span class="month">{{ $invoice->issued_at->translatedFormat('M') }}</span>
        <span class="year">{{ $invoice->issued_at->format('Y') }}</span>
      </div>
      @if($invoice->sale)
        <h4 style="margin-top:14px;">Vente associée</h4>
        <p style="font-weight:600;color:#1a237e;">{{ $invoice->sale->sale_number }}</p>
      @endif
    </div>
  </div>
  ...
  <div class="items-section">
    <table class="items-table">
      ...
      <tbody>
        ...
        @empty
          {{-- Fallback si pas de relation items mais qu'on a sale --}}
          @if($invoice->sale)
  ...
  <div class="amount-words">
    Arrêtée la présente facture à la somme de : <span>{{ \App\Helpers\NumberHelper::toWords($invoice->total_ttc) ?? number_format($invoice->total_ttc, 0, ',', ' ') . ' Francs CFA' }}</span>
  </div>
  ...
  <div class="payment-section">
    <div class="info-card">
      ...
      <div class="payment-methods">
        <div class="pay-item"><div class="pay-dot" style="background:#FF6900;"></div>Orange Money</div>
        <div class="pay-item"><div class="pay-dot" style="background:#1DC4C4;"></div>Wave</div>
        <div class="pay-item"><div class="pay-dot" style="background:#2D6CDF;"></div>Free Money</div>
        <div class="pay-item"><div class="pay-dot" style="background:#1a237e;"></div>Espèces</div>
      </div>
    </div>
